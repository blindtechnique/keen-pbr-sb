...
Список можно заполнить вручную или загрузить по URL. Поддерживаются домены, IP/CIDR и бинарные rule-set `.srs`, например:
...
В Release публикуются пакеты Entware для `aarch64-3.10`, `mips-3.4` и `mipsel-3.4`. На реальном роутере пока проверен только `aarch64-3.10`; сборки MIPS проходят автоматическую проверку формата IPK, комплектации и ELF-архитектуры, но до обратной связи от владельцев соответствующих устройств считаются экспериментальными.
