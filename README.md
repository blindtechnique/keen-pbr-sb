...
### Клиенты VPN-сервера роутера

Правила действуют не только на домашнюю сеть. Если на роутере поднят свой WireGuard-сервер, клиенты, подключившиеся к нему снаружи, ходят в интернет по тем же спискам и правилам: их трафик так же уходит в нужный туннель, а остальное — напрямую через nfqws2.

Прошивка подменяет адрес источника только для сетей, которые сама направила в туннель, поэтому для таких клиентов keen-pbr-sb делает это сам — на выходе каждого интерфейса, задействованного в outbound. Без этого пакет уходил бы в туннель с внутренним адресом вида `10.10.0.2`, и ответа бы не было. То же касается гостевых и любых других сегментов, о которых прошивка не знает.
