...
TEST_CASE("an out-of-order round cannot prune a newer interface lifetime") {
    Store store;
    store.begin_round(present({"eth0"}), at(0));

    // The newer worker completes first and sees a tunnel that was created
    // after the older worker began its dump.
    store.begin_round(present({"eth0", "tun0"}), at(20));
    store.observe_link_state("tun0", true, at(20));
    const auto fresh_anchor = store.anchor("tun0");
    REQUIRE(fresh_anchor.has_value());
    REQUIRE(fresh_anchor->up_since == at(20));

    // The old worker resumes with its stale membership snapshot.  It must be
    // rejected before pruning tun0 or inserting stale-only names.
    store.begin_round(present({"eth0", "stale0"}), at(10));

    const auto retained_anchor = store.anchor("tun0");
    REQUIRE(retained_anchor.has_value());
    CHECK(retained_anchor->up_since == at(20));
    CHECK_FALSE(store.anchor("stale0").has_value());
    CHECK(store.size() == 2);
}
