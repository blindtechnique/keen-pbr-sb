...
TEST_CASE("a request-scoped record is not kept alive by the API daemon") {
    TempDirectory directory;
    const auto path = directory.path / "transaction.json";
    ...
    const auto status = read_component_transaction(path);
    CHECK(status.state == ComponentTransactionState::abandoned);
    REQUIRE(status.record.has_value());
    CHECK_FALSE(status.record->owner_is_operation_process);
    // The writer PID is genuinely alive; only its lifetime is the wrong unit
    // of ownership for a request handler that may already have unwound.
    CHECK(status.record->owner_pid == static_cast<std::int64_t>(::getpid()));
}
...
TEST_CASE("a legacy daemon-owned request does not stay in flight forever") {
    TempDirectory directory;
    const auto path = directory.path / "legacy-owner.json";
    const auto current = sample_record();
    write_component_transaction(path, current);
    const auto parsed = read_component_transaction(path);
    REQUIRE(parsed.record.has_value());
    write_raw(path,
              "{\"version\":1,\"component\":\"nfqws2-keenetic\","
              "\"operation\":\"upgrade\",\"phase\":\"mutating\","
              "\"owner_pid\":" + std::to_string(parsed.record->owner_pid) +
              ",\"owner_start\":\"" + parsed.record->owner_start + "\"}");
    CHECK(read_component_transaction(path).state ==
          ComponentTransactionState::abandoned);
}
