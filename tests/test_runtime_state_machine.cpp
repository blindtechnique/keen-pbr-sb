...
TEST_CASE(
    "Meta QUIC recovery requires official primary and companion provenance in one rule") {
    for (const auto* primary_identity :
         {kInstagramCatalogIdentity, kMetaCatalogIdentity}) {
        CAPTURE(primary_identity);
        Config config;
        ListConfig primary;
        primary.catalog_identity = primary_identity;
        ListConfig companion;
        companion.catalog_identity = kWhatsappIpCatalogIdentity;
        config.lists = std::map<std::string, ListConfig>{
            {"friendly_primary", primary},
            {"friendly_ip_companion", companion}};

        RuleState rule;
        rule.rule_index = 0U;
        rule.action_type = RuleActionType::Mark;
        rule.fwmark = 0x00060000U;
        rule.list_names = {
            "friendly_primary", "friendly_ip_companion"};
        rule.criteria.dst_set_name = "kpbr4_friendly_primary";

        const auto scope =
            active_packaged_meta_quic_recovery_scope(config, {rule});
        const std::map<std::uint32_t, std::set<std::string>> expected{
            {0x00060000U, {"friendly_ip_companion"}}};
        CHECK(scope.companion_list_names_by_owned_mark ==
              expected);
    }
}

TEST_CASE("Meta QUIC recovery provenance and selection fail closed") {
    Config config;
    ListConfig named_only;
    ListConfig official;
    official.catalog_identity = kInstagramCatalogIdentity;
    ListConfig companion;
    companion.catalog_identity = kWhatsappIpCatalogIdentity;
    config.lists = std::map<std::string, ListConfig>{
        {"instagram", named_only},
        {"renamed_official", official},
        {"renamed_companion", companion}};

    const auto mark_rule = [](std::vector<std::string> lists,
                              std::uint32_t mark,
                              std::size_t index) {
        RuleState rule;
        ...
        rule.action_type = RuleActionType::Mark;
        rule.fwmark = mark;
        rule.list_names = std::move(lists);
        rule.criteria.dst_set_name = "kpbr4_scope";
        return rule;
    };

    SUBCASE("editable name without identity grants no authority") {
        const auto scope = active_packaged_meta_quic_recovery_scope(
            config,
            {mark_rule(
                {"instagram", "renamed_companion"},
                0x00060000U,
                0U)});
        CHECK(scope.empty());
    }

    SUBCASE("companion alone grants no authority") {
        const auto scope = active_packaged_meta_quic_recovery_scope(
            config,
            {mark_rule({"renamed_companion"}, 0x00060000U, 0U)});
        CHECK(scope.empty());
    }

    SUBCASE("separate rules do not combine even with the same mark") {
        const auto scope = active_packaged_meta_quic_recovery_scope(
            config,
            {mark_rule({"renamed_official"}, 0x00060000U, 0U),
             mark_rule({"renamed_companion"}, 0x00060000U, 1U)});
        CHECK(scope.empty());
    }

    SUBCASE("master recovery switch disables the policy") {
        config.daemon = DaemonConfig{};
        config.daemon->reconnect_unmarked_flows_on_routing_change = false;
        const auto scope = active_packaged_meta_quic_recovery_scope(
            config,
            {mark_rule(
                {"renamed_official", "renamed_companion"},
                0x00060000U,
                0U)});
        CHECK(scope.empty());
    }

    SUBCASE("explicit empty reconnect selection is an opt-out") {
        config.daemon = DaemonConfig{};
        config.daemon
    ...
    SUBCASE("explicit selection must include the companion list") {
        config.daemon = DaemonConfig{};
        config.daemon
            ->reconnect_owned_flows_on_routing_change_lists =
            std::vector<std::string>{"renamed_official"};
        const auto scope = active_packaged_meta_quic_recovery_scope(
            config,
            {mark_rule(
                {"renamed_official", "renamed_companion"},
    ...
    SUBCASE("explicit companion selection preserves exact mark association") {
        config.daemon = DaemonConfig{};
        config.daemon
            ->reconnect_owned_flows_on_routing_change_lists =
            std::vector<std::string>{"renamed_companion"};
        const auto scope = active_packaged_meta_quic_recovery_scope(
            config,
            {mark_rule(
                 {"renamed_official", "renamed_companion"},
             ...
             mark_rule(
                 ...
                 1U)});
        const std::map<std::uint32_t, std::set<std::string>> expected{
            {0x00060000U, {"renamed_companion"}},
            {0x00070000U, {"renamed_companion"}}};
        CHECK(scope.companion_list_names_by_owned_mark ==
              expected);
    }
}
