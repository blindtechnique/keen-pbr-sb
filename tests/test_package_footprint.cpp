...
TEST_CASE("a missing opkg record is empty, and says nothing about the package") {
    ...
    const auto strict =
        read_opkg_file_list_bounded(directory.path / "absent.list");
    CHECK_FALSE(strict.complete);
    CHECK_FALSE(strict.error.empty());
}

TEST_CASE("the mutation footprint refuses unsafe and excessive package paths") {
    TempDirectory directory;
    const auto unsafe = directory.path / "unsafe.list";
    write_file(unsafe, "/opt/usr/bin/nfqws2\n../outside\n");
    const auto rejected = read_opkg_file_list_bounded(unsafe);
    CHECK_FALSE(rejected.complete);
    CHECK(rejected.paths.empty());

    std::vector<std::string> excessive;
    excessive.reserve(kComponentMaxPathCount + 1U);
    for (std::size_t index = 0; index <= kComponentMaxPathCount; ++index)
        excessive.push_back("/bounded/path/" + std::to_string(index));
    const auto footprint = observe_package_footprint(excessive);
    CHECK_FALSE(footprint.complete);
    CHECK(footprint.files.empty());
}
