...
TEST_CASE("a forward permit reserves the last slot across concurrent callers") {
    const auto start = AuthForwardBudget::Clock::now();
    AuthForwardBudget budget(1U, 100s);
    auto first = budget.reserve_forward(start);
    REQUIRE(first.has_value());

    std::atomic<bool> second_admitted{true};
    std::thread second([&]() {
        const auto permit = budget.reserve_forward(start);
        second_admitted.store(
            permit.has_value(), std::memory_order_release);
    });

    // Commit the firmware-visible failure before releasing the reservation.
    // Whether the second thread is already queued or starts later, it cannot
    // observe the slot between the check and this record.
    first->record_forwarded_failure(start);
    first.reset();
    second.join();

    CHECK_FALSE(second_admitted.load(std::memory_order_acquire));
    CHECK(budget.spent(start) == 1U);
}

TEST_CASE("a successful forward permit returns its slot") {
    const auto start = AuthForwardBudget::Clock::now();
    AuthForwardBudget budget(1U, 100s);
    {
        auto permit = budget.reserve_forward(start);
        REQUIRE(permit.has_value());
        // No failure record models an authenticated firmware response.
    }
    CHECK(budget.reserve_forward(start).has_value());
    CHECK(budget.spent(start) == 0U);
}
