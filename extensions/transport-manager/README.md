...
Переключатель управляемого VPN в панели сохраняет включение или выключение в
`auto_start`: состояние сохраняется после перезапуска менеджера и роутера.
Перезапуск процесса и временная остановка при установке sing-box эту настройку
не меняют. Нативными интерфейсами по-прежнему управляет Keenetic/Netcraze.

## Режим процессов sing-box

Поле `sing_box_process_mode` по умолчанию имеет значение `isolated`: каждый
управляемый прокси работает в отдельном процессе и сохраняет нынешнюю границу
отказа. В режиме `shared` все активные управляемые прокси работают в одном
процессе sing-box, сохраняя отдельные TUN-интерфейсы и статусы API. Это уменьшает
расход памяти, но делает процесс общей границей отказа.

Переключение доступно через `PUT /v1/config/settings`. Сначала проверяется
полный общий конфиг, затем настройка сохраняется. Для безопасной смены объектов
lifecycle API возвращает `restart_required: true`: после этого нужно
перезапустить только transport-manager. При обновлении транспорта новый общий
конфиг проходит `sing-box check` до остановки живого процесса; неудачный запуск
автоматически возвращает предыдущий конфиг и процесс.
...
Для случайной проверки парсеров отдельно от сборки: `make transport-manager-fuzz`
из корня репозитория. Четыре Go `Fuzz*`-цели используют только синтетические
входы, не запускают сервисы и не обращаются к роутеру. Обычные `go test`
воспроизводят corpus; случайные мутации включаются только через `-fuzz`.
[Контракты, лимиты и команды FUZZ-01](../../docs/GO_FUZZING.ru.md).
...
Панель передаёт `X-KeenPbr-Persist-Enabled: 1` с действиями `up`/`down`, чтобы
сохранить выбранное состояние. Вызовы companion API без этого заголовка меняют
только текущее состояние и предназначены в том числе для временных остановок.
