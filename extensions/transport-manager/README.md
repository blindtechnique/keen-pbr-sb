...
Transport manager не меняет `ip rule`, nftables/iptables или системные маршруты.
sing-box только поднимает именованный TUN. Выбор трафика, urltest и failover
настраиваются в `keen-pbr`. Адрес прокси-сервера следует исключить из
проксируемых списков, чтобы не создать маршрутную петлю.
