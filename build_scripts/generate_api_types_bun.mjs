...
try {
  ...
  // чтение — неопределённое поведение. UBSan поймал ровно это:
  // «load of value 49, which is not a valid value for type 'bool'» при
  // копировании InternalVpnServerElement. Инициализатор в объявлении делает
  // весь класс ошибки невозможным, независимо от того, как объект создан.
  //
  // Перечисления намеренно не трогаем: `= {}` дало бы им первый по порядку
  // элемент, а это уже выбор значения по смыслу, а не защита от мусора.
  content = content.replace(
