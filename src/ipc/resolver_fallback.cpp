...
bool emit_resolver_fallback(std::ostream& output,
    ...
    while (input.read(hash_buffer, sizeof(hash_buffer)) ||
           input.gcount() != 0) {
        md5.update(
            reinterpret_cast<const std::uint8_t*>(hash_buffer),
    ...
    output << "# keen-pbr resolver state: fallback reason="
           << reason_code << '\n';
    output << "txt-record=config-hash.keen.pbr," << timestamp << "|"
           << crypto::digest_to_hex(md5.digest()) << '\n';
    output << "txt-record=resolver-state.keen.pbr," << timestamp
