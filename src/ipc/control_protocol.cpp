...
nlohmann::json decode_message(const std::string& frame) {
    ...
    if (frame.size() != sizeof(encoded_length) + payload_size) {
        throw ControlProtocolError(
            "control message length does not match frame");
    }

    try {
        return nlohmann::json::parse(
            frame.data() + sizeof(encoded_length),
            frame.data() + frame.size());
    } catch (const nlohmann::json::exception& error) {
        throw ControlProtocolError(
            "invalid control JSON: " + std::string(error.what()));
    }
}
...
void validate_request_envelope(const nlohmann::json& request) {
    ...
    if (!request.contains("request_id") ||
        ...
        throw ControlProtocolError(
            "control request requires a non-empty request_id");
    }
