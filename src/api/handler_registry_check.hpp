...
// control, so it is opt-in per request and never scheduled: without
// `allow_external_lookup` nothing leaves the router. The answer is about the
// registry, not about this router - whether the domain actually fails here is
// what the routing test and the reachability probes are for, and the two are
