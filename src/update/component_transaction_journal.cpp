...
ComponentTransactionStatus read_component_transaction(
    ...
    // A directory, symlink or device at the durable journal path is not an
    // absent record and must never authorize another package mutation.
    ...
    try {
        ...
        // Stream failures (including a concurrent path replacement) and JSON
        // failures are both an unreadable durable record, never absence.
        status.state = ComponentTransactionState::unreadable;
        return status;
    }
    if (!parsed.is_object() ||
    ...
    try {
        ...
        record.owner_is_operation_process =
            parsed.value("owner_is_operation_process", false);
    ...
    // the operator about.
    status.state = record.owner_is_operation_process &&
                           owner_is_alive(record.owner_pid, record.owner_start)
...
bool clear_component_transaction(const std::filesystem::path& path) {
    ...
    if (!std::filesystem::exists(file_status)) return true;
    if (!std::filesystem::is_regular_file(file_status)) return false;

    // Retain the record bytes until the unlink is durably committed. If the
    // directory fsync fails, republish them best-effort so a reported clear
    // failure does not look like "no transaction" to the next request.
    std::string body;
    try {
        std::ifstream input(path, std::ios::binary);
        ...
        if (input.bad() || body.empty() || body.size() > 64U * 1024U)
            return false;
    } catch (const std::exception&) {
        // The path may have changed type after the no-follow status check, or
        // the filesystem may have failed mid-read. Neither is proof that the
        // durable transaction record can be safely cleared.
        return false;
    }

    if (!std::filesystem::remove(path, error) || error) return false;
    if (sync_directory(path)) return true;
    ...
    try {
        ...
        // A committed atomic write may throw only because its own directory
        // fsync failed; in that case the record is at least visible again.
    }
    return false;
}
