...
inline constexpr std::size_t kComponentMaxPathCount = 512;
inline constexpr std::size_t kComponentMaxPathLength = 1024;
inline constexpr std::uintmax_t kComponentMaxFileBytes = 16U * 1024U * 1024U;
inline constexpr std::uintmax_t kComponentMaxTotalBytes = 64U * 1024U * 1024U;
...
struct PackageFootprint {
    ...
    // False means observation itself exceeded a bound or encountered a path
    // that cannot safely be represented. A caller must not turn a partial
    // observation into a rollback promise.
    bool complete{true};
...
struct PackagePathList {
    bool complete{false};
    std::vector<std::string> paths;
    std::string error;
};

// Strict, bounded form for mutation paths. Unlike the compatibility helper
// above, a missing or oversized package record is an explicit refusal rather
// than an empty list that could be mistaken for a tiny package.
