...
private:
    ...
    // Observation rounds are assembled by concurrent API/SSE workers.  A
    // worker can finish an older netlink dump after a newer one has already
    // been committed, so the store must reject that whole stale membership
    // snapshot before it can prune or insert anything.
    std::optional<TimePoint> newest_round_at_;
