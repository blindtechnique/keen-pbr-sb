...
bool RuntimeStateMachine::transition(RuntimeState next,
                                     std::string reason,
                                     std::string& error) {
    const bool allowed =
        ...
        (state_ == RuntimeState::running &&
         (next == RuntimeState::applying || next == RuntimeState::restart_required ||
          next == RuntimeState::stopped || next == RuntimeState::broken ||
          next == RuntimeState::shutting_down)) ||
        (state_ == RuntimeState::restart_required &&
         (next == RuntimeState::applying || next == RuntimeState::stopped ||
          next == RuntimeState::broken || next == RuntimeState::shutting_down)) ||
        (state_ == RuntimeState::applying &&
         (next == RuntimeState::running || next == RuntimeState::broken ||
          next == RuntimeState::stopped)) ||
        (state_ == RuntimeState::stopped &&
         (next == RuntimeState::starting || next == RuntimeState::applying ||
          next == RuntimeState::broken || next == RuntimeState::shutting_down)) ||
        (state_ == RuntimeState::broken &&
         (next == RuntimeState::starting || next == RuntimeState::applying ||
          next == RuntimeState::stopped || next == RuntimeState::shutting_down)) ||
        (state_ == RuntimeState::shutting_down && next == RuntimeState::stopped);
    if (!allowed) {
        error = std::string("invalid runtime transition: ") +
                runtime_state_name(state_) + " -> " + runtime_state_name(next);
        return false;
    }
    state_ = next;
