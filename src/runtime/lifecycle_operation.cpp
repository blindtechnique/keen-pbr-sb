...
void LifecycleOperationCoordinator::start_stage(const std::string& id,
                                                const std::string& stage_id) {
    mutate(id, [&](auto& operation) {
        const auto it = std::find_if(operation.stages.begin(), operation.stages.end(),
                                    [&](const auto& stage) { return stage.id == stage_id; });
        if (it != operation.stages.end() &&
            it->status == LifecycleOperationStatus::Pending) {
            it->status = LifecycleOperationStatus::Running;
        }
    });
}

void LifecycleOperationCoordinator::succeed_stage(const std::string& id,
                                                  const std::string& stage_id,
                                                  std::string detail) {
    mutate(id, [&](auto& operation) {
        for (auto& stage : operation.stages) {
            if (stage.id != stage_id || stage.status != LifecycleOperationStatus::Running) continue;
...
void LifecycleOperationCoordinator::fail_stage(const std::string& id,
                                               const std::string& stage_id,
                                               std::string detail) {
    mutate(id, [&](auto& operation) {
        for (auto& stage : operation.stages) {
            if (stage.id != stage_id ||
                (stage.status != LifecycleOperationStatus::Pending &&
                 stage.status != LifecycleOperationStatus::Running)) continue;
...
void LifecycleOperationCoordinator::skip_stage(const std::string& id,
                                               const std::string& stage_id,
                                               std::string detail) {
    mutate(id, [&](auto& operation) {
        for (auto& stage : operation.stages) {
            if (stage.id != stage_id || stage.status != LifecycleOperationStatus::Pending) continue;
            stage.status = LifecycleOperationStatus::Skipped;
            stage.detail = std::move(detail);
            break;
        }
