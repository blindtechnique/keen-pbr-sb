...
struct TtlBypassInputs {
    ...
    // Proven from the live nfqws-owned mangle rules. A package/binary using a
    // different processed bit must not inherit a fleet-wide RETURN for the
    // historical default.
    bool nfqws_processed_mark_observed{false};
    bool nfqws_processed_mark_compatible{false};
    // keen-pbr routing marks must not own the processed bit either; otherwise
    // ordinary policy-routed packets could bypass the firmware TTL chain.
    bool routing_mark_disjoint{true};
};
