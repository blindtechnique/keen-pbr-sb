...
TtlBypassPlan plan_ttl_bypass(const TtlBypassInputs& inputs) {
    ...
    if (!inputs.nfqws_processed_mark_observed) {
        plan.state = TtlBypassState::unknown;
        plan.detail = "could not inspect the live nfqws processed mark";
        return plan;
    }

    if (!inputs.routing_mark_disjoint ||
        !inputs.nfqws_processed_mark_compatible) {
        plan.state = TtlBypassState::conflict;
        plan.detail = !inputs.routing_mark_disjoint
            ? "keen-pbr routing mark overlaps the nfqws processed bit"
            : "the live nfqws processed mark is absent or incompatible";
        // A previously safe rule becomes unsafe when either mark contract
        // changes. Remove only our tagged copies and stay fail-closed.
        for (std::size_t i = 0; i < count_our_rules(inputs.chain_rules); ++i) {
            plan.commands.push_back(with_prefix(
                {"-t", "mangle", "-D", kTtlChain}, rule_body()));
        }
