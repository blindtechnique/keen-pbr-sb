...
- **P1 / nfqws rotator:** по live-наблюдению на `Dimkin-S26U` приложение
  ...
  проходят; новый IPK и восстановление выученного слота после restart/reboot
  ещё не проверялись.
...
- Обычная загрузка Keenetic больше не исчерпывает короткий интерактивный
  ...
  фонового обслуживания. Этот срез устраняет starvation обычного START, но сам
  по себе не считается аппаратной cold-boot приёмкой первичного выбора.
- URLTEST-группы при START/cold boot больше не публикуются с пустым
  ...
  собран; новая IPK и аппаратная cold-boot приёмка ещё не выполнялись.
- Доменные назначения DNS-серверов из системной политики KeeneticOS больше не
  ...
  assertions, generator — 10, scoped group — 15; проверка diff чистая. Новая
  IPK и router acceptance после настоящего reboot ещё не выполнялись.
