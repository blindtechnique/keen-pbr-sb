...
- Проверка пути маршрутизации в этой карточке теперь показывает три отдельных
  решения: выбранное правило, опубликованный firewall outbound с точной меткой
  и read-only ответ FIB ядра с интерфейсом и таблицей. Явный запрет маршрута,
  отсутствие применимости и невозможность получить надёжный ответ различаются;
  последняя не выдаётся пользователю за поломку маршрутизации. Тот же typed
  результат проходит через OpenAPI и управляющий Unix-сокет, не создавая
  временных firewall-объектов.
...
- При неудачном «Применить и перезапустить» панель явно сообщает, что рабочая
  ...
  проверку переводов.
- Для внутренней службы OpenConnect снятая галочка «через keen-pbr-sb» больше
  не отключает доменные и IP-правила маршрутизации. В этом режиме клиенты
  OpenConnect по-прежнему классифицируются по спискам назначения, но их DNS к
  точному адресу роутера не попадает под принудительный redirect. Подтверждённые
  интерфейс и пул сохраняются при кратковременной недоступности NDMS; другие
  внутренние VPN-службы не изменены.
