...
Формат основан на Keep a Changelog. Новые сборки обозначаются как `3.3.2-ГГГГММДДччммсс`: первая часть — базовая версия, последние 14 цифр — время исходного коммита в UTC.

## [Unreleased] · Alpha

- Добавлена совместимость с nfqws2-keenetic 1.3.1: стандартная конфигурация распознаётся после настройки WAN/IPv6, проверка PPE допускает дополнительные перехватываемые порты, не расширяя активные селекторы. Неподдерживаемый `reload` удалён из панели и отвергается API до изменений службы. Safe/Balanced/Max сохранены; проверки Lua выполняются со старой и новой версиями движка.
- В Alpha можно сохранять VPN или группу для скачивания обновлений из панели и штатного консольного установщика. Недоступность выбранного пути не вызывает прямую загрузку; HTTPS, подпись и проверка свободного места сохранены. Настройка не управляет первоначальным скачиванием установщика и зависимостями `opkg`.
- Улучшены проверка собранной конфигурации nfqws и пояснения ошибок: пустые действия, неверные и повторные объявления blob отклоняются до применения, включая повторы между TCP/QUIC-профилями.
- README Alpha выбирает последний опубликованный подписанный выпуск без устаревающего номера в команде установки и объясняет ожидание незавершённой GitHub-сборки.

## [3.3.2-20260922160538 · Alpha](https://github.com/blindtechnique/keen-pbr-sb/releases/tag/alpha-35752719907-1)

- Добавлен раздел «VPN для устройств»: интернет-трафику выбранного клиента по закреплённому IPv4 можно назначить отдельный VPN или группу. Новое правило по умолчанию имеет приоритет над правилами сайтов. IPv6 не покрывается, привязка не следует за MAC, DHCP и DNS не меняются. После сохранения нужно применить изменения; старые соединения не сбрасываются, при отказе VPN действует политика маршрута.
- README и сайт дополнены описанием VPN для устройства и пошаговой установкой: Entware и HTTPS, ответы установщику, пароль, первый вход, проверка доступа и помощь при ошибках. README Alpha содержит актуальный подписанный выпуск, команды последней Alpha и отдельную команду для точного выпуска.

- В Alpha исправлен учёт раннего закрытия TLS-соединения в общем TCP-пуле nfqws Max: закрытие сервером без ответа после ClientHello учитывается при выборе следующего варианта. Смена варианта сохраняет порог двух неудачных соединений; повторные пакеты, отмена запроса клиентом и закрытие после ответа не добавляют ложных отказов.
- В настройках обновления добавлен постоянный выбор Stable/Alpha: сохранение канала не устанавливает пакет и не меняет VPN/DNS. Показаны выбранный и установленный каналы, полные версии и источник. Установка привязана к подтверждённым тегу и версии подписанного выпуска; изменение канала или выпуска требует новой проверки, понижение версии из панели запрещено. Обновление и пакетный откат не сбрасывают выбранный канал.
- Опубликован подписанный Alpha-набор для `aarch64-3.10`, `mips-3.4`, `mipsel-3.4`: перед подписью проверены комплектность, общая версия, исходный коммит и канал каждого пакета. Новая публикация через устаревший `next` удалена; проверка исторических подписей сохранена. Успешная сборка не заменяет аппаратную приёмку на конкретной модели.
- В Alpha обновление из панели сохраняет канал установленного пакета: выбирает Alpha-релиз и проверяет подпись именно этого канала. Старый кэш main и недоступность GitHub не вызывают перехода на main. Канал показан рядом с установленной версией.
- Общий неопределённый результат фонового обновления firewall больше не выдаётся за сбой Meta/WhatsApp без признаков затронутой политики. Исходная причина сохраняется в подробностях; безопасная повторная проверка и ограничения очистки соединений остаются в силе.
...
- Отдельная очередь регулярных проверок туннелей сохраняет свежие наблюдения при долгих ручных проверках и фоновых задачах.
- Ручное сравнение сайтов и ресурсов по выбранным путям и семействам IPv4/IPv6: до 12 последовательных проб без включения выключенных VPN. Счётчики правил доступны в расширенной диагностике по запросу и не подменяют RX/TX интерфейсов.
- В Balanced/Max расширено окно обнаружения поздних TCP-отказов; профиль Safe сохранён. Исправлены наблюдение за обновлением при временной потере панели и лишняя повторная авторизация.
- Добавлены fuzz-тесты парсеров VPN и подписок, исправлен разбор некорректного TUIC-входа. Методики нагрузки и исследование list:set опубликованы отдельно; экспериментальный backend не включён в рабочую маршрутизацию.

## Изменения тестовой ветки

- Добавлен раздел «VPN для устройств» для закреплённого IPv4: выбор известного устройства или ручной ввод, отдельный VPN либо группа, приоритет над правилами сайтов, изменение и удаление привязки. До сохранения форма не меняет конфигурацию; после него используется штатный черновик и применение. IPv6 не покрывается, DHCP и DNS автоматически не меняются. Устаревший список устройств, конфликт адресов и повторная привязка явно обрабатываются.

В этой ветке находятся экспериментальные изменения. Понятная сводка новой версии и история выпусков доступны в [основном журнале изменений](https://github.com/blindtechnique/keen-pbr-sb/blob/main/CHANGELOG.md).
