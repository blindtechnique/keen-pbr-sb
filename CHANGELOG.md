...
- Удаление не требует повторного ввода пароля администратора внутри уже авторизованной сессии только из-за того, что операция относится к native VPN.
- Если Keenetic уже удалил native-интерфейс, панель автоматически завершает служебную запись, сбрасывает кэш и убирает исчезнувшую строку без отдельной кнопки «восстановления» и без повторных подтверждений.
...
- Восстановлено плавное перемещение индикатора вкладок в стиле KeeneticOS: глобальная настройка сокращения движения больше не превращает его в мгновенный прыжок.
