...
- Обычная загрузка Keenetic больше не исчерпывает короткий интерактивный
  ...
  фонового обслуживания, поэтому ошибка SNAT/DNS/catalog scheduler не оставляет
  готовые дочерние туннели без выбранного выхода.
- Доменные назначения DNS-серверов из системной политики KeeneticOS больше не
  отбрасываются. Когда Keenetic DNS участвует в fallback, привязка домена или
  суффикса, например `youtube.com`, переносится в dnsmasq и имеет приоритет над
  общими upstream-серверами, поэтому запрос не уходит параллельно на
  настроенный в keen-pbr `8.8.8.8`. Для одного домена зашифрованные назначения
  предпочитаются обычным. Явное DNS-правило keen-pbr сохраняет приоритет над
  пересекающейся системной привязкой, а публичный scoped upstream наследует
  настроенный detour, не превращая локальные DNS-прокси Keenetic во внешний
  трафик.
