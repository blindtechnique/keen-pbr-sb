...
- **Вход учётной записью роутера.** keen-pbr-sb может проверять логин и пароль через саму прошивку Keenetic (challenge-ответ на `/auth`), не храня пароль у себя. Способ входа переключается в настройках; учётные данные проверяются до сохранения, чтобы нельзя было заблокировать себе доступ.
- **Вложенные failover-группы.** Группа urltest может содержать другие группы: маршрутизация рекурсивно спускается по выбору до конечного интерфейса, косвенные циклы обрываются ограничителем глубины, прямая самоссылка отклоняется валидатором.
- **Каталог готовых списков** — 77 наборов правил sing-box по категориям, подставляются в поле URL при создании списка. Источник подборки — [hoaxisr/awg-manager](https://github.com/hoaxisr/awg-manager).
- Подсветка синтаксиса в редакторах конфигов nfqws2, списков и журналов.
- Кнопки перезапуска keen-pbr-sb, sing-box и nfqws2 в карточке служб.
...
- Раздел «DNS-правила» убран: основные DNS-серверы переехали в «DNS-серверы», а правило для списка задаётся при его редактировании.
- Соединения сгруппированы по устройствам с раскрытием сессий, как в интерфейсе роутера.
- Расписание автообновления списков задаётся выбором периодичности вместо cron-выражения; режим cron остался для сложных случаев.
- Списки на узких экранах больше не требуют горизонтальной прокрутки, страница Outbounds разбита на группы.
- Глобальная блокировка при падении туннеля убрана из настроек: поведение по умолчанию зависит от типа выхода и переопределяется в самом outbound.
