...
- **P1 / nfqws rotator:** по live-наблюдению на `Dimkin-S26U` приложение YouTube через QUIC работает, но `m.youtube.com` по TCP зависал при подтверждённо исправных системном и роутерном DNS. Реализовано сохранение между перезапусками nfqws только подтверждённых успешных слотов `gv_tcp` / `yt_tcp` / `yt_quic`: снимок привязан к точному `kpbr_rev`, ограничен 64 записями и 30 днями, использует debounce 120 секунд и не более одной записи за 6 часов; счётчики ошибок не сохраняются. Для `yt_tcp` первая неудача засчитывается только после `retrans=2`, после переключения счётчик сбрасывается для быстрой повторной проверки. Узкие тесты Lua, генератора и `postinst` проходят; IPK-приёмка на роутере после перезагрузки ещё не выполнена.
