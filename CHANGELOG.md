...
- Опциональный режим `daemon.meta_udp443_policy: messages_first` отклоняет только UDP/443 к адресам штатного IP-компаньона Meta/WhatsApp с меткой его маршрута. После этого клиент может перейти на TCP, что может повысить надёжность доставки сообщений. Режим не блокирует UDP/3478, UDP/5349 и P2P-медиа на высоких портах. По умолчанию действует `balanced` без блокирующих правил; при включении интерфейс предупреждает о влиянии на Instagram и другие сервисы Meta, возможной дополнительной задержке звука звонка на 10–20 секунд и необходимости отключённой IPv6-маршрутизации до появления проверенного IPv6-покрытия. Owned-правило проверяется и удаляется также после аварийного завершения демона и при удалении пакета.
