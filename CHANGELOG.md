...
- Завершение native-импорта больше не теряет введённые псевдоним, выбор
  ...
  только несекретный план оформления; URI и ключи в него не попадают.
- Псевдоним импортируемого WG/AWG теперь передаётся в фактически используемое
  ...
  комментарий `# Name`. Поэтому описание интерфейса в самом Keenetic начинается
  с выбранного человеком имени; точный служебный суффикс остаётся для обычного
  удаления через корзину.
- Для native WG/AWG результат разрешённого автоопределения страны сохраняется в
  ...
  адреса в несекретном native-tracker.
