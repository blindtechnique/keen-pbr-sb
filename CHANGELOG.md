...
- Если Keenetic принял одноразовый WG/AWG import и потребовал фоновое завершение,
  окно теперь закрывается сразу после передачи запроса. Привязка маршрута,
  применение конфигурации и автоопределение страны продолжаются на странице
  списка и больше не ждут ручного закрытия модального окна.
