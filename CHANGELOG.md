...
- **Карточка точек выхода на дашборде переписана.** В строке было пять способов сказать примерно одно: цветная точка, значок типа, имя ядра и «Исправен» — и всё это повторялось у каждого участника группы. Осталось четыре факта: жив ли, как называется, чем работает, как быстро отвечает. Порядок обхода показывается только у групп резервирования, а имена ядра ушли со страницы совсем — они нужны при настройке, а дашборд отвечает на другой вопрос.
