...
## [3.3.2 Alpha] — 2026-09-15

- Для регулярной диагностики туннелей выделена ограниченная очередь: долгие ручные проверки и фоновые задачи не должны лишать исправные соседние туннели свежих наблюдений.
- В Balanced/Max увеличено окно штатного автодетектора nfqws2 для поздних TCP-отказов. Safe сохранён; новые домены не добавляются вручную и оригинальный бинарник nfqws2 не заменяется.
- Добавлено ручное сравнение сайтов и отдельных ресурсов через выбранные пути и IPv4/IPv6: до 12 последовательных проб, без запуска выключенных VPN и автоматического изменения маршрутов.
- Показатели правил вынесены в расширенную диагностику дашборда. Запрос только по кнопке, честный частичный учёт и отдельные физические записи; RX/TX интерфейсов не подменяются per-rule статистикой.
- Добавлены Go fuzz-тесты парсеров VPN-ссылок/подписок и исправлен найденный некорректный TUIC-вход; включена ограниченная CI-проверка синтетических входов.
- Опубликованы методики нагрузочной проверки и исследование `list:set`. Последнее не включено в рабочий firewall: сначала нужны измерения на Keenetic.
- Новая программа и точный IPK требуют отдельной приёмки на тестовом роутере. Подписанная Alpha не становится Stable/Latest автоматически.

## [3.3.2 Alpha] — 2026-09-14

- Перенесены актуальные исправления main: маршрутизация при обновлении списков и сохранение выключенных подключений.
- Исправлены повторная авторизация перед обычным обновлением и восстановление наблюдения за установкой после временного исчезновения панели.
- Добавлены профильная диагностика nfqws2 и подсказки по спискам.
- Доработаны детектор TCP-ротатора, отдельная обработка googlevideo и экспериментальные SYN/Discord-пулы Max.
- Исправлены ABI connndmmark на Keenetic и изоляция fake-пакетов от настоящего TCP-потока; системные библиотеки и оригинальный бинарник nfqws2 не заменяются.
- Alpha получает собственный подписанный prerelease и установку через `wget` или `curl` с явным `--alpha`. Stable/Latest не меняется.
- Техническое описание, команды и границы проверки: [README](README.md).
