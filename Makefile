...
transport-manager-build: transport-manager-test ## Cross-compile transport manager for supported Keenetic architectures
	...
	cd $(TRANSPORT_MANAGER_DIR) && CGO_ENABLED=0 GOOS=linux GOARCH=mipsle GOMIPS=softfloat \
		go build -trimpath -buildvcs=false -ldflags='-s -w' \
		-o ../../$(TRANSPORT_MANAGER_DIST_DIR)/transport-manager-mipsel ./cmd/transport-manager
	cd $(TRANSPORT_MANAGER_DIR) && CGO_ENABLED=0 GOOS=linux GOARCH=mips GOMIPS=softfloat \
		go build -trimpath -buildvcs=false -ldflags='-s -w' \
		-o ../../$(TRANSPORT_MANAGER_DIST_DIR)/transport-manager-mips ./cmd/transport-manager
	cd $(TRANSPORT_MANAGER_DIR) && CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=7 \
		go build -trimpath -buildvcs=false -ldflags='-s -w' \
		-o ../../$(TRANSPORT_MANAGER_DIST_DIR)/transport-manager-armv7 ./cmd/transport-manager
	cd $(TRANSPORT_MANAGER_DIR) && CGO_ENABLED=0 GOOS=linux GOARCH=arm64 \
		go build -trimpath -buildvcs=false -ldflags='-s -w' \
		-o ../../$(TRANSPORT_MANAGER_DIST_DIR)/transport-manager-aarch64 ./cmd/transport-manager
	cd $(TRANSPORT_MANAGER_DIR) && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
		go build -trimpath -buildvcs=false -ldflags='-s -w' \
		-o ../../$(TRANSPORT_MANAGER_DIST_DIR)/transport-manager-x64 ./cmd/transport-manager
...
NARROW_TEST_TARGETS := \
	...
	keen-pbr-internal-vpn-resolution-cache-tests \
...
test-package-dns: ## Run isolated Keenetic DNS and uninstall regressions without compiling
	python3 -m unittest build_scripts.tests.test_update_download_transport -v
