export const ruTranslation = {
  subscriptions: {
    title: "Подписки",
    description:
      "Срок действия и трафик от провайдера. Обновление сведений не меняет VPN и маршруты.",
  ...
  pages: {
    ...
    settings: {
      ...
      general: {
        ...
        tunnelProbeHelp:
          "Эта настройка двигает трафик сама, и обратной дороги на практике нет: правила nfqws2 привязаны к провайдерскому интерфейсу, поэтому уведённый хост исчезает из его поля зрения и перестаёт давать то свидетельство, по которому его туда отправили. Подтверждённые хосты дописываются в собственный файл списка — ваша конфигурация не переписывается никогда. Хост уводится, только если проба говорит, что он заблокирован именно здесь, и реестр блокировок называет его: на живом трафике именно второй вопрос отделяет заблокированный сайт от рекламного адреса, который не отвечает по своим причинам.",
