    ...
    settings: {
      ...
      general: {
        ...
        tunnelProbeOutboundHint:
          "Можно не заполнять — тогда возьмётся первое исходящее соединение с интерфейсом. Указывайте тег того, у которого интерфейс есть: каждая проба привязывается к устройству, и исходящее с одной лишь меткой пробу не понесёт. Через него же пойдёт и найденное — маршрутизировать хост через другой туннель означало бы, что измерение ничего не обосновало.",
