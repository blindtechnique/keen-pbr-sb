    ...
    settings: {
      ...
      general: {
        ...
        tunnelProbeOutboundHint:
          "Тег исходящего соединения, у которого есть интерфейс. Каждая проба привязывается к устройству, поэтому исходящее с одной лишь меткой не подойдёт.",
        tunnelProbeListLabel: "Список для найденного",
        tunnelProbeListHint:
          "Имя списка, у которого задан файл. Дописывается именно в этот файл, поэтому список, хранящий хосты только в конфигурации, здесь не подходит. Файл создайте заранее: если он указан в списке и не существует, разбор списков бросает ошибку внутри применения файервола, и маршрутизация не применится целиком. Пустого файла достаточно, а домены в самом списке можно оставить — источники читаются вместе.",
