export const ruTranslation = {
  nfqws: {
    ...
    upgradeTip: {
      ...
      blocked: "Сейчас обновить нельзя: роутер не берётся сделать это безопасно.",
      metadataUnverified:
  ...
  transports: {
    ...
    nativeImport: {
      ...
      description:
        "Импорт доступен после входа через защищённый HTTPS-домен Keenetic или прямое локальное подключение, подтверждённое роутером. Предварительный структурный разбор выполняется только в этом браузере: приложение не отправляет исходный текст, URI и ключи по сети и не сохраняет их. В превью значения ключей скрыты, но endpoint может отображаться. Создание интерфейса в Keenetic пока отключено.",
      fileDescription:
        "Выберите или перетащите клиентский файл WireGuard/AmneziaWG. Разбор выполняется только в этом браузере; исходный файл и ключи не отправляются по сети.",
      ...
      redactedNotice:
        "Это локальное предварительное структурное превью, а не подтверждение готовности конфигурации. Значения ключей скрыты, но endpoint может отображаться. Предложенное имя никуда не записано и должно быть явно принято или изменено при будущем сохранении. Применение по-прежнему отключено.",
      ...
      createOnlyRange:
        "Для будущего импорта разрешён только новый интерфейс в диапазоне {{first}}–{{last}}; существующие и защищённые интерфейсы изменяться не должны.",
    ...
    singBoxInstall: {
      ...
      stop: "Остановить",
      tipStop:
        "Остановит установку. На роутере пока ничего не изменилось.",
    ...
    subscriptionImport: {
      ...
      discardConfirm:
        "Закрыть импорт и отбросить текущий выбор?",
      expired:
        "Предпросмотр истёк — демон хранит его десять минут.",
      expiredRefetch: "Загрузить заново",
      problemsSummary:
        "Записей в списке, требующих внимания: {{count}}.",
      urlRefused: {
        scheme_not_allowed:
  ...
  overview: {
    ...
    routeTraffic: {
      ...
      idleCounters: "Используемых интерфейсов с нулевыми счётчиками: {{count}}.",
      unavailableCounters:
    ...
    targetFacts: {
      ...
      nfqws: {
        ...
        unknown: "Не удалось безопасно прочитать настройки nfqws или активный список — покрытие неизвестно.",
        covered: "Цель есть в активных списках nfqws — обход DPI к ней применяется.",
        excluded:
