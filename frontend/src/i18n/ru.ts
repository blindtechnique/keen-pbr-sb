    ...
    settings: {
      ...
      advanced: {
        ...
        metaUdp443PolicyHint:
          "Настройка действует только для трафика, направленного через keen-pbr-sb и покрытого штатным дополнительным IP-списком Meta/WhatsApp.",
        ...
        whatsappTcpResetSourcesLabel:
          "Устройства для экспериментальной ротации TCP WhatsApp",
        whatsappTcpResetSourcesPlaceholder: "192.168.1.44\n192.168.1.45",
        whatsappTcpResetSourcesHint:
          "Введите до {{max}} точных IPv4-адресов устройств через запятую или с новой строки. Закрепите эти адреса за устройствами в DHCP, иначе защита перестанет соответствовать телефону после смены IP. Пустой список оставляет ротацию выключенной. CIDR, диапазоны и режим для всех устройств намеренно недоступны.",
        whatsappTcpResetSourcesWarningTitle:
          "Может затронуть Instagram; только iptables",
        whatsappTcpResetSourcesWarningDescription:
          "Роутер не определяет приложение Android внутри общих адресных диапазонов Meta. Одноразовый сброс применяется только к точному простаивающему TCP-потоку указанного устройства, но этот поток может принадлежать Instagram или другому сервису Meta. Функция работает только с бэкендом iptables и остаётся выключенной для неуказанных устройств.",
        whatsappTcpResetSourcesValidationInvalid:
          "{{value}} — не точный одноадресный IPv4-адрес. CIDR и диапазоны недопустимы.",
        whatsappTcpResetSourcesValidationDuplicate:
          "Адрес {{value}} указан больше одного раза.",
        whatsappTcpResetSourcesValidationTooMany:
          "Укажите не более {{max}} устройств (сейчас {{count}}).",
