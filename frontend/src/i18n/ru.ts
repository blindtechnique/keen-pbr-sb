export const ruTranslation = {
  ...
  auth: {
    ...
    protectedTransportTitle: "Откройте панель напрямую из локальной сети",
    protectedTransportRequired:
      "Учётные данные роутера можно вводить только через прямое локальное подключение, подтверждённое самим роутером. Это внешнее или проксированное подключение не может их запрашивать.",
  ...
  transports: {
    ...
    nativeImport: {
      title: "Импорт WireGuard / AmneziaWG из файла",
      description:
        "Через авторизованное прямое локальное подключение, подтверждённое роутером, выберите либо перетащите клиентский файл .conf. Предварительный структурный разбор выполняется только в этом браузере: приложение не отправляет исходный текст и ключи по сети и не сохраняет их. В превью значения ключей скрыты, но endpoint может отображаться. Создание интерфейса в Keenetic пока отключено.",
      transportBlockedTitle: "Для импорта нужен защищённый вход",
      transportBlockedDescription:
        "Откройте панель по локальному адресу управления роутером. Импорт и drag-and-drop доступны только после подтверждения роутером авторизованного прямого локального подключения. Внешний или проксированный HTTP заблокирован; локальная сеть всё равно должна быть защищена от перехвата.",
      dropzoneLabel: "Выбрать или перетащить файл конфигурации WireGuard",
      dropzone: "Перетащите .conf сюда или нажмите, чтобы выбрать",
      fileHint: "Один текстовый файл .conf, не больше {{size}} КиБ",
      clear: "Убрать выбранный файл",
      errorTitle: "Файл не принят",
      redactedNotice:
        "Это локальное предварительное структурное превью, а не подтверждение готовности конфигурации. Значения ключей скрыты, но endpoint может отображаться. Применение по-прежнему отключено.",
      apply: "Создать интерфейс в Keenetic",
      applyBlockedTitle: "Создание интерфейса пока отключено",
      applyBlockedDescription:
        "Подтверждённый роутером локальный канал превью уже активен, но пока нет API для передачи ключей и применения, типизированных команд KeeneticOS, автоматической резервной копии, проверки владельца, защиты от параллельных изменений и отката. Применить файл к роутеру нельзя.",
      ...
      errors: {
        ...
        "conf-extension-required": "Нужен файл с расширением .conf.",
        ...
        unsupported_uri:
          "В это поле нужен обычный .conf. Импорт URI будет доступен вместе с безопасным backend-применением.",
        invalid_base64: "Один из ключей WireGuard имеет неверный формат.",
