export const ruTranslation = {
  nfqws: {
    ...
    upgradeUnavailableDescription:
      "Панель не запустит opkg, пока не сможет сохранить точный прежний IPK, проверить и закрепить целевой IPK, сохранить состояние opkg и завершить откат после перезагрузки. Текущая конфигурация не изменяется.",
    ...
    restoreComponentConfirmTitle: "Вернуть файлы nfqws2, сохранённые перед обновлением?",
    restoreComponentConfirmDescription:
      "Сохранённые бинарь, конфигурация и списки будут записаны обратно. Если служба работала до возврата, панель запустит её на восстановленном бинаре и проверит NFQUEUE; если была остановлена — оставит остановленной.",
    restoreComponentLimitTitle: "Чего это не делает",
    restoreComponentLimitDescription:
      "Файлы, добавленные новым пакетом, останутся на месте. Это возврат nfqws2 к сохранённым файлам, а не роутера к точному прежнему состоянию.",
    ...
    serviceHelp: {
      ...
      upgrade:
        "Ставит новую версию самого nfqws2 из репозитория. Кнопка доступна только при наличии точного и переживающего перезагрузку отката.",
      captureRestorePoint:
  ...
  transports: {
    ...
    nativeInterface: {
      ...
      uptime: "На связи",
      uptimeFromFirmware: "По данным KeeneticOS: переживает перезапуск keen-pbr",
