export const ruTranslation = {
  nfqws: {
    ...
    upgradeUnavailableDescription:
      "Backend не разрешил это обновление пакета. Обновите страницу; если блокировка останется, проверьте состояние операции с пакетом перед новой попыткой.",
    upgradeMetadataUnverifiedDescription:
      "Сохранённая операция nfqws2 означает, что восстановленные файлы и метаданные opkg могут относиться к разным версиям. Проверка и установка обновлений из панели останутся заблокированы, пока вы вручную не восстановите пакет через opkg по SSH.",
    ...
    maintenanceHint: "Редкие действия: пакет, точки возврата, резервные копии",
    serviceHelp: {
      ...
      maintenance:
        "Редкие действия под одной кнопкой: обновление пакета, точки возврата до и после него, резервные копии и перечитывание конфигурации без остановки. Каждое объяснено в самом меню.",
      restart:
        "Останавливает nfqws и запускает заново с текущей конфигурацией. Соединения на секунду прерываются, зато применяется всё, что изменено.",
      reload:
        "Служба перечитывает файлы настроек, не останавливаясь: соединения не рвутся. Подходит, когда поменялись только списки и параметры.",
      upgrade:
        "Просит текущие настроенные opkg-источники Entware выбрать и установить nfqws2-keenetic под общей блокировкой. Панель сама не закрепляет и независимо криптографически не проверяет целевой пакет или источник. Перед изменением она сохраняет файлы, но не обеспечивает точный откат пакета или восстановление после потери питания.",
      captureRestorePoint:
        "Сохраняет нынешние файлы nfqws2 — бинарь, конфигурацию и списки — чтобы можно было к ним вернуться. Обновление делает это само; эта кнопка нужна, чтобы взять точку возврата перед любыми другими изменениями.",
      restoreComponent:
        "Возвращает сохранённые файлы nfqws2 и проверяет исходное состояние службы, бинарь процесса и NFQUEUE. Это не точный откат пакета opkg.",
      backup:
        "Сохранить настройки, списки, Lua-скрипты и стратегии в файл на этом компьютере — или восстановить их из такого файла.",
      refresh:
