...
/**
 * Доли трафика по маршрутам.
 *
 * Считаем принятое плюс отправленное по интерфейсу, на который маршрут
 * приземляется. Счётчик накопительный — с момента поднятия интерфейса, а не за
 * сутки: постоянного учёта у нас нет, история это кольцевой буфер на 120 точек,
 * который наполняется только пока открыта панель. Поэтому кольцо отвечает на
 * «куда ушёл трафик с тех пор, как всё поднялось», и подпись обязана говорить
 * ровно это.
 *
