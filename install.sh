...
if [ "$UPDATE_ONLY" = "1" ]; then
    ...
    # Измерено 30.08.2026: демон, поднятый изнутри postinst, может оказаться не в
    # состоянии применить ни один выбор urltest и сам из этого не выходит - в
    # каждой urltest-таблице остаётся один unreachable, и трафик помеченных в эти
    # группы списков уходит в никуда. Подробности в комментарии в postinst.
    #
    # Обновление раньше ставило пакет и сразу выходило, поэтому роутер оставался
    # в этом состоянии до следующего ручного перезапуска. Заявляем о своём
    # перезапуске заранее: postinst увидит переменную и пропустит собственную
    # страховку, чтобы служба не перезапускалась дважды подряд.
    KEEN_PBR_INSTALLER_RESTARTS=1
    export KEEN_PBR_INSTALLER_RESTARTS
    install_package_transactionally
    say "Обновление установлено, перезапускаю службу..."
    if ! /opt/etc/init.d/S80keen-pbr restart; then
        die "не удалось перезапустить keen-pbr после обновления"
    fi
    say "Обновление keen-pbr-sb завершено, служба перезапущена."
    exit 0
fi
choose_sing_box
repair_interrupted_nfqws_bootstrap
